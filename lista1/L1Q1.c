...
typedef struct start {
  int sum;
  int num[25];
  // int *num = malloc(25 * sizeof(int));
  int numCounter;
} start;

void initSStart(start *numbers, int alocSize) {
...
void initSStart2(start *numbers, int alocSize) {
  for (int i = alocSize; i < (alocSize + 10); i++) {
    numbers[i].numCounter = 0;
    numbers[i].sum = 0;
    int j = 0;
    while (j < 25) {
...
void separateNumbersAndSave(FILE *input_file, FILE *output_file) {
...
int main(void) {
  ...
  FILE *output_file = fopen("L1Q1.out", "w");

  if (!readf("L1Q1.in", &input_file) || !output_file) {
    printf("Closing...\n");
    return EXIT_FAILURE;
  }

  separateNumbersAndSave(input_file, output_file);
